...
When Caddy, Traefik, Zoraxy, Cloudflare, or another reverse proxy forwards traffic to
...
the real client IP for OIDC, MCP OAuth, and other rate limits.
...
If the Docker host `resolv.conf` has a `search` domain (common on Proxmox),
wildcard DNS for that domain can steal container names such as
`opnform-client`. The Compose file sets `dns_search: [.]` so lookups stay on
the Docker network. Nginx upstreams also use trailing-dot names
(`opnform-client.`, `opnform-api.`). If the ingress is unhealthy and logs show
timeouts to a Cloudflare or public IP on port 3000, this is the cause.
