...
On OpnForm Cloud, guest form creation and preview remain available before authentication. On self-hosted instances, guest draft creation is unavailable. Schema resources, the field catalog, and `validate_form_definition` remain available before authentication because they do not read or store account data. Creating or listing forms requires OAuth: Cursor (or another MCP client) should open a browser to this instance so you can sign in and approve access. You do not create a Google or GitHub OAuth app for that flow.
...
Enabling MCP only exposes the server. Complete the OAuth flow from the MCP client. Guest drafts on OpnForm Cloud do not authenticate your account automatically. You do not create a Google, GitHub, or other identity-provider app. OpnForm uses Laravel Passport: the client should open a browser to your instance, you sign in with your existing OpnForm account, then approve access.

Dynamic client registration is not a completed login. If the client shows connected after a register call but never opens `/oauth/authorize`, disconnect and authenticate again, then finish the browser consent screen. For local Codex testing, start a new conversation after authentication.
</Accordion>
...
On the standard Docker ingress, `/mcp`, `/oauth/`, and `/.well-known/oauth-*` must reach the API. `GET /mcp/authorize` stays on the frontend for the consent screen.

If another reverse proxy (Zoraxy, Caddy, Traefik, Cloudflare) sits in front of the ingress, forward those paths without blocking hidden URLs, and disable response buffering for `/mcp`. Set `TRUSTED_PROXIES` in `api/.env` to that proxy's IP so Laravel honors `X-Forwarded-Proto`.
</Accordion>
</AccordionGroup>
