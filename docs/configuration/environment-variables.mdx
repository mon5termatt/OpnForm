...
Upstream OpnForm can collect anonymous usage statistics. **This fork defaults telemetry to off.**

**How to keep it disabled (default):**

Leave unset, or set `OPNFORM_ANONYMOUS_TELEMETRY_DISABLED=true`.

**How to opt in:**

Set `OPNFORM_ANONYMOUS_TELEMETRY_DISABLED=false` if you explicitly want to send anonymous usage events to OpnForm.

**When telemetry can be active (only if opted in):**
...
None of your customer data is transmitted. Stats would cover basic feature usage and an anonymous instance UUID only (no form content, submissions, or user emails).
