...
vi.mock('~/composables/useInstanceLicense.js', async () => {
  const vue = await import('vue')

  return {
    useInstanceLicense: () => ({
      isLicenseBypass: vue.computed(() => mocks.isLicenseBypass),
    }),
  }
})
  ...
  return mount(UserSettingsModal, {
    ...
    global: {
      stubs: {
        ...
        LazyUsersSettingsUpstreamUpdates: true,
      },
    },
  })
}
...
describe('User settings instance administration', () => {
  ...
  it('shows instance settings when the user administers another workspace', () => {
    ...
    expect(wrapper.find('[data-page-id="upstream-updates"]').exists()).toBe(false)
  })

  it('shows the Updates tab when license bypass is active', () => {
    mocks.isLicenseBypass = true
    mocks.workspaces = [
      mocks.currentWorkspace,
      { id: 2, is_admin: true, is_readonly: false },
    ]
    ...
    expect(wrapper.find('[data-page-id="upstream-updates"]').exists()).toBe(true)
  })
